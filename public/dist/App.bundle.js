...
function typeAhead(search) {
	...
	searchInput.on('input', function (e) {
		...
		_axios2.default.get('/api/search?q=' + this.value).then(function (res) {
			if (res.data.length) {
				results.innerHTML = _dompurify2.default.sanitize(searchResultsHTML(res.data));
				return;
			}

			results.innerHTML = _dompurify2.default.sanitize('<div class="search__result">No search results found for ' + _this.value + '</div>');
		}).catch(function (err) {});
	});
...
function loadPlaces(map) {
	var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 28.637255099999997;
	var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : -81.38379299999997;
